...
export class DOMUtils {
   ...
   */
  static toggleElementVisibility(element: HTMLElement): void {
    if (!element) {
   ...
   */
  static toggleIcon(icon: HTMLElement | null): void {
    if (!icon) {
   ...
   */
  static addTemporaryClass(element: HTMLElement, className: string, duration: number = TIMEOUTS.BUTTON_FEEDBACK): void {
...
export class ErrorHandler {
   ...
   */
  static logSuccess(context: string, message: string): void {
    const sanitizedContext = String(context).replace(/[\r\n]/g, '');
    const sanitizedMessage = String(message).replace(/[\r\n]/g, '');
